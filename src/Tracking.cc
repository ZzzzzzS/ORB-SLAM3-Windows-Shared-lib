...
namespace ORB_SLAM3
{
    ...
    Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
    {
        ...
        //  双目模式，注意跟两个相机模式区分开
        if (mSensor == System::STEREO && !mpCamera2) // 平行双目模式
            mCurrentFrame = Frame(
                mImGray,                                 // 左目图像
                imGrayRight,                             // 右目图像
                timestamp,                               // 时间戳
                ...
                mThDepth,                                // 远点,近点的区分阈值
                mpCamera);                               // 相机模型
        else if (mSensor == System::STEREO && mpCamera2) // 真双目模式
        ...
        mCurrentFrame.mnDataset = mnNumDataset; // 好像没啥用
    ...
    void Tracking::Track()
    {
        ...
        if (mState != NO_IMAGES_YET)
        {
            ...
            else if (mCurrentFrame.mTimeStamp > mLastFrame.mTimeStamp + 1.0) // 得注意这个时间戳的情况！
            {
        ...
        if (mState == NOT_INITIALIZED)
        {
            ...
            {
                // 双目RGBD相机的初始化共用一个函数
                StereoInitialization(); // TODO: 看初始化
            }
            else
            {
                // 单目初始化
            ...
            if (!mbOnlyTracking)
            {
                ...
                if (mState == OK)
                {
                    ...
                    if ((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId < mnLastRelocFrameId + 2)
                    {
                        ...
                        bOK = TrackReferenceKeyFrame(); //TODO: 看
                    }
                    else
                    {
                        ...
                        bOK = TrackWithMotionModel(); //TODO:看
                        if (!bOK)
